...
export default function DiagnosticBilling() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase
        ...
        .select('id, name, default_price, extra_copies_json');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/print/diagnostic/${bill.id}`);
    } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">ডায়াগনস্টিক বিলিং</h1>
        <button
          onClick={() => navigate('/history?type=diagnostic')}
      ...
      {/* Referred By, Ref No, Date, Time */}
      <div className="bg-white rounded-xl shadow-sm border p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">রেফার্ড বাই</label>
          <input
            type="text"
